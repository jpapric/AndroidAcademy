...
class ProductListViewModel(private val productRepository: ProductRepository) : ViewModel() {
        ...
        viewModelScope.launch {
            ...
            productRepository.getProducts()
                ...
                .onFailure { throwable ->
                    ...
                    _productListUIState.value = when {
                        throwable.isNoInternetError() -> ProductListUIState.NoInternet
                        else -> ProductListUIState.Error("We couldn't load the product list.")
                    }
                }
        ...
        viewModelScope.launch {
            productRepository.addProduct(
                ...
                .onFailure { throwable ->
                    ...
                    _productOperationState.value = ProductOperationState.Error(
                        throwable.toOperationMessage("Adding product failed")
                    )
    ...
    fun editProduct(
        ...
        viewModelScope.launch {
            productRepository.updateProduct(
                ...
                .onFailure { throwable ->
                    _productOperationState.value = ProductOperationState.Error(
                        throwable.toOperationMessage("Editing product failed")
                    )
    ...
    fun deleteProduct(
        ...
        viewModelScope.launch {
            productRepository.deleteProduct(id = id)
                ...
                .onFailure { throwable ->
                    _productOperationState.value = ProductOperationState.Error(
                        throwable.toOperationMessage("Deleting product failed")
                    )
    ...
    private fun Throwable.toOperationMessage(defaultMessage: String): String {
        return when {
            isNoInternetError() -> "Check your internet connection and try again."
            this is IllegalArgumentException -> message ?: defaultMessage
            else -> defaultMessage
        }
    }
